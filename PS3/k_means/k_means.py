...
def init_centroids(num_clusters, image):
    ...
    # *** START YOUR CODE ***
    centroids_init = []
    pixels = [ [x,y] for x in range(image.shape[0]) for y in range(image.shape[1]) ]
    np.random.shuffle(pixels)
    for i in range(num_clusters):
        centroids_init.append(image[pixels[i][0],pixels[i][1],:])

    # *** END YOUR CODE ***

    return np.array(centroids_init, dtype=np.float)
