...
def initialize_mdp_data(num_states):
    ...
    # *** START CODE HERE ***
    value_func = np.random.random(num_states)/10
    transition_counts = np.zeros((num_states, num_states, 2)) ## for the 2 actions
    transition_probs = np.ones((num_states, num_states, 2)) / num_states
    reward_counts = np.zeros((num_states, 2)) ## for count of new state that gave reward and the reward
...
def choose_action(state, mdp_data):
    ...
    # *** START CODE HERE ***
    value0 = mdp_data['transition_probs'][state, :, 0].dot(mdp_data['value'])
    value1 = mdp_data['transition_probs'][state, :, 1].dot(mdp_data['value'])
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    for action in [0, 1]:
        for state in range(mdp_data['num_states']):
            total_state_transitions = np.sum(mdp_data['transition_counts'][state, :, action ])
            if total_state_transitions != 0:
                mdp_data['transition_probs'][state, :, action] = (mdp_data['transition_counts'][state, :, action ]/total_state_transitions)

    for state in range(mdp_data['num_states']):
        if mdp_data['reward_counts'][state, 1] > 0:
            mdp_data['rewards'][state] = (mdp_data['reward_counts'][state, 0]/mdp_data['reward_counts'][state, 1])
    # *** END CODE HERE ***
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    while True:
        ...
        num_iter = num_iter + 1
        for state in range(mdp_data['num_states']):
            value1 = mdp_data['transition_probs'][state, :, 0].dot(mdp_data['value'])
            value2 = mdp_data['transition_probs'][state, :, 1].dot(mdp_data['value'])

            new_value[state] = max(value1, value2)

        new_value = mdp_data['rewards'] + gamma * new_value

        max_diff = float('-Inf')
        for state in range(mdp_data['num_states']):
            if abs(new_value[state] - mdp_data['value'][state]) > max_diff:
                max_diff = abs(new_value[state] - mdp_data['value'][state])
